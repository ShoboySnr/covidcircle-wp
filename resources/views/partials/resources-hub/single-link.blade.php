<div class="read-more-link content-bg relative h-full flex" onclick="location.href='{{ the_permalink() }}'">
  <div class="text-container-bg">
    <div class="tag flex justify-between items-start w-full">
      <?php
        $categories = Utility::getSingleCustomCategory('resources_categories');

        if(!empty($categories)) {
      ?>
        <a href="{{ the_permalink() }}" title="{{ $categories[0]['name'] }}">{{ $categories[0]['name'] }}</a>
        <?php } else {
          ?>
        <div></div>
        <?php
    ...
    <h2>
      {{ Utility::limitStr(the_title('', '', false), 100) }}
    </h2>
    <a href="{{ get_permalink() }}" title="Read more..." class="flex read-more">
